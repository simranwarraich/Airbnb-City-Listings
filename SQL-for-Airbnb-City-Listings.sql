...
select count(*) as listings,concat('$',round(AVG(price),2) ) as Average_price , round(AVG(number_of_reviews),2) as average_reviews  from AirBnb_NewYork_City_Listings.listings where price>0;
...
BEGIN
SELECT neighbourhood_group, neighbourhood,count(id) as no_of_listings, concat("$",ROUND(AVG(price),2)) AS average_price,min(price) as min_price, max(price) as max_pricve,ROUND(AVG(number_of_reviews),2) as average_reviews,
